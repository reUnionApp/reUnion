...
function AllActivities(props) {
  ...
  return (
    <div>
      <h2>You have {props.activities.length} {props.activities.length > 1 ? "activities" : "activity"}</h2>
      <ul>
        {props.activities &&
          props.activities.map((activity) => {
            return (
              <div key={activity.id}>
                <Link to={`/myEvents/${props.match.params.eventId}/activities/${activity.id}`}>
