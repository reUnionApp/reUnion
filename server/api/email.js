...
router.post("/invitation", async (req, res, next) => {
  ...
  console.log('emailsSent before sending emails', emailsSent)
  ...
  `,
    html: `<!DOCTYPE html><html><head><meta charset="utf-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><title></title><meta name="description" content=""><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href=""></head><body style="font-family:"Segoe UI", Tahoma, Geneva, Verdana, sans-serif;"><div><h2 style="margin: 2rem 0;">You\'re Invited! Get excited!</h2><div><p>${req.user.firstName} invited you to the event ${req.body.event.eventName}</p><div style="text-align: center; margin: 5rem auto; "><div style="margin: auto;"><p><span style="font-weight: bold;">Event Name:</span> ${req.body.event.eventName}</p><p><span style="font-weight: bold;">Event Start Date:</span> ${dateFormat(new Date(req.body.event.startDateTime))}</p><p><span style="font-weight: bold;">Event Start Time:</span> ${new Date(req.body.event.startDateTime).toLocaleTimeString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit' })}</p><p><span style="font-weight: bold;">Event End Date:</span> ${dateFormat(new Date(req.body.event.endDateTime))}</p><p><span style="font-weight: bold;">Event End Time:</span> ${new Date(req.body.event.endDateTime).toLocaleTimeString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit' })}</p><p><span style="font-weight: bold;">Location:</span> ${req.body.event.location}</p><p><span style="font-weight: bold;">Description:</span> ${req.body.event.description}</p></div><div><p>To respond to this invitation, <a href="https://myreunionapp.herokuapp.com/">click here</a></p><p>https://myreunionapp.herokuapp.com</p></div></div></div></div></body></html>`,
  }
  sgMail
    .send(msg)
    .then(() => {
  ...
  console.log('emailsSent after sending emails', emailsSent)

  console.log('after updateInviteStatus', finalList.Events)
  ...
  console.log('after updateInviteStatus', finalList)
});
